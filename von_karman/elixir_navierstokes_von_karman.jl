...
boundary_conditions = Dict(
   ...
   :Right => boundary_condition_subsonic,
...
lift_force_viscous = AnalysisSurfaceIntegralViscous(indices, TrixiLW.LiftForceViscous(
   aoa, rho_inf(), 1.0, linf()))
drag_force_viscous_corr = TrixiLW.AnalysisSurfaceIntegralViscousCorrectedGrad(indices, TrixiLW.DragForceViscous(
   aoa, rho_inf(), 1.0, linf()))
lift_force_viscous_corr = TrixiLW.AnalysisSurfaceIntegralViscousCorrectedGrad(indices, TrixiLW.LiftForceViscous(
   aoa, rho_inf(), 1.0, linf()))
...
analysis_callback = AnalysisCallback(semi, interval=1,
                                     ...
                                     analysis_integrals = (
				       drag_force_viscous_corr, lift_force_viscous_corr,
				       # drag_force_viscous, lift_force_viscous,
                                       drag_force, lift_force)
				     )

alive_callback = AliveCallback(analysis_interval=2000)
...
###############################################################################
# run the simulation
# time_int_tol = 5e-10 # For N = 4, 5e-10 works well
time_int_tol = 1e-8 # For N = 4, 5e-10 works well
# time_int_tol = 1e-11 # For N = 5
cfl_number = 500.0
